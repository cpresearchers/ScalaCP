...
//    println("c_sum = " + c_sum)
    ...
    while (parallelism <= maxPara) {
      ...
      println(s"${parallelism}线程 ${ppType} ${heuName}===============>")
      for (i <- 1 to exe) {
        val pct = new IPTotalCoarseSolver(xm, parallelism, ppType, varType, heuName)
        pct.search(Constants.TIME)
        pct.shutdown()
        node = pct.helper.nodes
        time += pct.helper.time
    ...
    while (parallelism <= maxPara) {
      ...
      println(s"${parallelism}线程 ${ppType} ${heuName}===============>")
      for (i <- 1 to exe) {
        val pct = new IPSingleCoarseSolver(xm, parallelism, ppType, varType, heuName)
        pct.search(Constants.TIME)
        pct.shutdown()
        node = pct.helper.nodes
        time += pct.helper.time

...
class PWRSBitSet (id: Int, numTuples: Int, numVars: Int) {
  ...
  val Delta =  Array.fill(num_bit)(0L)

  if (lastLimits != 0){
    words(0)(num_bit - 1) <<= 64 - lastLimits
    Delta(num_bit - 1) <<= 64 - lastLimits
  }
  //初始化limit, index, mask
  // rint
...
//        PWwords(currentLevel)(offset) = words(preLevel)(offset)
        i -= 1
      }
      preLevel = level
  ...
  def intersectWithMask(): Boolean = {
    ...
    while (i >= 0) {
      ...
      if (w != currentWords) {
        ...
        Delta(offset)=words(currentLevel)(offset) & ~w     //获取CT删除的元组

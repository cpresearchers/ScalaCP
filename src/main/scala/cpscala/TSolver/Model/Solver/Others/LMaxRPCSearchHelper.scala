...
class LMaxRPCSearchHelper(override val numVars: Int, override val numTabs: Int, xm: XModel) extends SearchHelper(numVars, numTabs) {
  ...
  val subscription = Array.fill(numTabs)(new ArrayBuffer[Propagator])
  // 为两两变量间生成中间变量的矩阵
  val commonVar = Array.fill(numVars)(Array.fill(numVars)(new ArrayBuffer[BitSetVar_LMRPC]))
  // 为两两变量间生成共同约束的矩阵
  val commonCon: Array[Array[ArrayBuffer[Propagator]]] = Array.fill(numVars)(Array.fill(numVars)(new ArrayBuffer[Propagator]))
  // 存储一个变量所有的临域

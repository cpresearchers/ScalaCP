...
public class CPFSolverImpl_with_relation extends CPFSolver {
    ...
    private ArrayList<Integer> Select_Path()
    {
        ...
        while(num < tabsize)
        {
            ...
            for(var j : zm.tabs.get(r.get(r.size() - 1)).scope)
            {
       ...
       // super(xm, varType, heuName,sear);
        ...
        for (var P : Path) {
            ...
            if (P.id != p.get(0)) {
                ...
                Trie_Data T = new Trie_Data( -P.id,zm.tabs.get(P.id).scope);
                for (int i = 0; i < zm.tabs.get(P.id).R.rvs.length; ++i) {
                    ArrayList<Integer> s = new ArrayList<>();
                    for (var same : Path_Diff.get(Path_Diff.size() - 1).same_id) {
                        s.add(zm.tabs.get(P.id).R.rvs[i][same]);
                    }
    ...
    public  boolean Search(long limit_time) {
        ...
        long current_Time = 0;
        while(table_flag[0] < zm.tabs.get(Path.get(0).id).R.rvs.length && !flag_for_Solution)
        {
    ...
    public boolean Assignment(final int level, int[] solution, int[] table_flag,ArrayList<Integer> p) {
        ...
        if(level == 0)
        {
            for(int i = 0;i < Path.get(level).scope.size();++i)
                solution[Path.get(level).scope.get(i)] = zm.tabs.get(Path.get(level).id).R.rvs[table_flag[level]][i];
        ...
        if(j < p.size())
        {
            ...
            {
                solution[Path.get(level).scope.get(i)] = zm.tabs.get(Path.get(level).id).R.rvs[p.get(j)][i];
            }

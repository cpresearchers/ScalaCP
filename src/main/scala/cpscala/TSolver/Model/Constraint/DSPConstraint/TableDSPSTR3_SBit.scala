...
class TableDSPSTR3_SBit(val id: Int, val arity: Int, val numVars: Int, val scope: Array[PVar], val tuples: Array[Array[Int]], val helper: DSPSearchHelper) extends DSPPropagator {
  ...
  private[this] val StackS: Array[RestoreStack[Int, Int]] = Array.fill(arity)(new RestoreStack[Int, Int](numVars))

  // 无效元组
  private[this] val invalidTuples: SparseSetInt = new SparseSetInt(tuples.length, numVars)

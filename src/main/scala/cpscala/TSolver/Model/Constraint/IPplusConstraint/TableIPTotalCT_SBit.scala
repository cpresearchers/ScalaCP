...
class TableIPTotalCT_SBit(val id: Int, val arity: Int, val numVars: Int, val scope: Array[PVar], val tuples: Array[Array[Int]], val helper: IPplusSearchHelper) extends IPplusPropagator {
  val currTab = new RSBitSet(id, tuples.length, numVars)

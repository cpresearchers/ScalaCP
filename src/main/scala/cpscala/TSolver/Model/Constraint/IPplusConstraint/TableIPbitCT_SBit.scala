...
class TableIPbitCT_SBit(val id: Int, val arity: Int, val num_vars: Int, val scope: Array[PVar], val tuples: Array[Array[Int]], val helper: IPplusSearchHelper) extends IPplusPropagator {
  ...
  def updateTable(): Boolean = {
    ...
    while (i < Sval.length && helper.isConsistent) {
      ...
      if (currTab.isEmpty()) {
        ...
        failWeight += 1
        //        atomFailWeight.incrementAndGet()

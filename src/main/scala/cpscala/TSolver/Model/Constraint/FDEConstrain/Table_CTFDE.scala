...
class Table_CTFDE(val id: Int, val arity: Int, val num_vars: Int, val scope: Array[Var], val tuples: Array[Array[Int]], val helper: SearchHelper) extends FDEPropagator {

  val currTab = new FDEBitSet(id, tuples.length, num_vars)
  val supports = new Array[Array[ArrayBuffer[Long]]](arity)
  val localsupport=new Array[Array[ArrayBuffer[Int]]](arity)
  val num_bit = currTab.numBit
  val residues = new Array[Array[Int]](arity)
  level = 0

  for (i <- 0 until arity) {
    supports(i) = new Array[ArrayBuffer[Long]](scope(i).size)
    localsupport(i)=new Array[ArrayBuffer[Int]](scope(i).size)
    for(j<-0 until scope(i).size){
      supports(i)(j)=new ArrayBuffer[Long]()
      localsupport(i)(j)=new ArrayBuffer[Int]()
    }
...
//      supports(j)(t(j))(x) |= Constants.MASK1(y)
      if(localsupport(j)(t(j)).contains(x)){
        supports(j)(t(j))(localsupport(j)(t(j)).indexOf(x)) |= Constants.MASK1(y)
      }else{
        localsupport(j)(t(j))+=x
        supports(j)(t(j))+=Constants.MASK1(y)
      }
    }
    ii += 1
  }
  ...
  def updateTable(): Boolean = {
    ...
    while (i < Sval.length) {
      ...
      if (numRemoved >= numValid || firstPropagate) {
        ...
        for (a <- values) {
          currTab.addToMask(localsupport(vv)(a),supports(vv)(a))
        }
      } else {
  ...
  def filterDomains(y: ArrayBuffer[Var]): Boolean = {
    ...
    for (vv <- Ssup) {
      ...
      for (a <- values) {
        ...
        if (index == Constants.kINDEXOVERFLOW || (currTab.words(helper.level)(index) & supports(vv)(a)(localsupport(vv)(a).indexOf(index))) == 0L) { //res失效
          index = currTab.intersectIndex(localsupport(vv)(a),supports(vv)(a))
          if (index != -1) { //重新找到支持
            residues(vv)(a) = index
          }
          else {
